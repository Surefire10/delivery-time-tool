...
export function eachHourOfInterval({
  ...
  // Adjust start and end to top of hour in UTC
  const adjusted = {
    start: subMinutes(startOfHour(start.getTime()), offset.start),
    end: subMinutes(startOfHour(end.getTime()), offset.end),
  };

  let accumulator = adjusted.start;
  const hours: Date[] = [];

  while (accumulator <= adjusted.end) {
    hours.push(accumulator);
    accumulator = addHours(accumulator, 1);
  }
